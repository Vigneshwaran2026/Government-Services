(function ($) {
  ...
  // Date Picker
  if ($(".lotech-datepicker").length) {
    $('.lotech-datepicker').datetimepicker({
      timepicker: true,
      datepicker: true,
      format: 'y-m-d H:i',
      hours12: false,
    ...
    $(".count-box").appear(
      function () {
        var $t = $(this),
          ...
          r = parseInt($t.find(".count-text").attr("data-speeds"), 10);
  ...
  //accrodion
  if ($(".lotech-accrodion").length) {
    var accrodionGrp = $(".lotech-accrodion");
        ...
        $(this)
          ...
          .on("click", function () {
            if ($(this).parent().hasClass("active") === false) {
              $(".lotech-accrodion." + accrodionName)
                .find(".accrodion")
                .removeClass("active");
              $(".lotech-accrodion." + accrodionName)
                .find(".accrodion")
  ...
  function thmOwlInit() {
    // owl slider
    let lotechowlCarousel = $(".lotech-owl__carousel");
    if (lotechowlCarousel.length) {
      lotechowlCarousel.each(function () {
        let elm = $(this);
    ...
    let lotechowlCarouselNav = $(".lotech-owl__carousel--custom-nav");
    if (lotechowlCarouselNav.length) {
      lotechowlCarouselNav.each(function () {
        let elm = $(this);
  ...
  function lotechSlickInit() {
    // slick slider
    let lotechslickCarousel = $(".lotech-slick__carousel");
    if (lotechslickCarousel.length) {
      lotechslickCarousel.each(function () {
        let elm = $(this);
        let options = elm.data("slick-options");
        let lotechslickCarousel = elm.slick(
          "object" === typeof options ? options : JSON.parse(options)
        );
      });
    }
    let lotechslickCarouselCounter = $(".lotech-slick__custome-counter");
    if (lotechslickCarouselCounter.length) {
      lotechslickCarouselCounter.each(function () {
        let elm = $(this);
        let options = elm.data("slick-options");
        let currentSlide;
        let slidesCount;
        let sliderCounter = document.createElement('div');
        sliderCounter.classList.add('lotech-slick__counter');

        let updateSliderCounter = function (slick, currentIndex) {
          currentSlide = slick.slickCurrentSlide() + 1;
          slidesCount = slick.slideCount;
          $(sliderCounter).html('<span class="lotech-slick__counter__active">' + currentSlide + '</span>' + '' + '<span>' + slidesCount + '</span>')
        };
  ...
  function lotech_stretch() {
    var i = $(window).width();
    $(".row .lotech-stretch-element-inside-column").each(function () {
      var $this = $(this),
  ...
  $(document).ready(function () {
    $('.portfolio-three__wrapper .portfolio-three__item').on("mousemove", function () {
      $('.portfolio-three__wrapper .portfolio-three__item').removeClass("active");
      $(this).addClass("active");
    });
  });
